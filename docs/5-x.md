...
- 数据报头部的组成
    ...
    - Differentiated services 字段（8 bit）
        ...
        - 现在前6位用于标记数据包的service class，包括 expedited & assured services（见教材5.4.5，先前跳过了）
    ...
    - MF（1 bit）代表More Fragments（更多片段）。使用非透明传输时，最后一个片段的这一位设为0，其余片段设为1。
    ...
    - Options 字段填充为4字节（32 bit）的倍数。协议设计时，定义了下面五个选项。
        ...
        - Record route选项要求路径上的每个路由器将其IP地址附加到Options字段中，现在已经不够用了。
...
- NAT（Network Address Translation，网络地址转换）
    ...
    - NAT带来的问题：
        ...
        - NAT打破了端到端的连接模型：在发送传出数据包之前，无法接受传入数据包
