...
- Acknowledgement number（确认号）为 cumulative acknowledgement（累积确认）。
    ...
    - 重申：TCP流中的每个字节都被编号。
???+ info "举例"
    将某个TCP连接的 initial sequence number（初始序列号）记为$ISN$，则$ISN+i$代表数据流的第$i$个字节。
    
    如果接收方连续收到的两个段的 sequence number 分别为$ISN+i_1$和$ISN+i_2$，且先前所有段都传输成功，则第一个段中传输的数据的地址范围是$i_1$到$i_2-1$，接收端在对第一个数据段的回复中，acknowledgement number 应该填写$ISN+i_2$。
...
- 改进前面提到的 three-way handshake（三次握手）
    ...
    - 如果双方使用同一对套接字同时向对方建立连接，由于连接由 endpoint（port）标识，只建立一个连接
        ...
        ![image-20241219185631093](static/image-20241219185631093.png)
...
![image-20241219191515446](static/image-20241219191515446.png)
...
- 算法流程
    - slow start（慢启动）
        - 拥塞窗口从较小的初始值（一般为1个 packet 的大小）开始。
        - 每收到一个确认（或者说每隔一个RTT），拥塞窗口**增加一倍**的大小。
