...
- 算法流程
    ...
    - slow start（慢启动）
        ...
        - 数据包发送速度逐渐加快，直到拥塞窗口大小超过 ssthresh（slow start threshold，慢启动阈值），或者检测到拥塞，或者接收方窗口已满。
            - 初始化时，ssthresh 设为流量控制窗口的大小（也可以任意高）。
            - 检测到数据包丢失（超时）时，慢启动阈值设置为拥塞窗口的一半，然后拥塞窗口重置为其小初始值，并重新开始慢启动
...
- 传输速度的计算：
    ...
    - tramsmission time = window size / channel bandwidth
    - data transmission rate = window size / (RTT + transmission time)
- 回绕对传输速度的限制：
    - 最大传输速度 = $2^{sequence\ number}$ * 帧大小 / MSL
        - 32即TCP头部中的序列号长度
        - 这里的MSL与前面连接的断开那里的MSL是同一个概念